<template>
  <div class="myhotplace-list">
    <div v-if="filteredPosts.length === 0" class="no-data-msg">좋아요한 핫플레이스가 없습니다.</div>
    <div v-else>
        ...
        >
          ...
          <div class="hotplace-card-body">
            ...
            <div class="hotplace-card-info">
              ...
              <span>
                <i class="fa-solid fa-heart" style="color: #ed4856"></i>&nbsp;{{ post.likeCount }}
              </span>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async fetchLikedHotplacePosts() {
      const authStore = useAuthStore()
      try {
        const res = await axios.get('/api/hotplace/posts/liked', {
          headers: {
  ...
  async mounted() {
    await this.fetchLikedHotplacePosts()
  },
}
</script>
